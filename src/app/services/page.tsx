  ...
  return (
    <>
              ...
              <ul className="space-y-3">
                {["24/7 Emergency response capability", "ISO 27001 certified operations", "Bilingual adjusters (English/Spanish)", "Transparent reporting and communication"].map((item, i) => (
                  <li key={i} className="flex items-center gap-3 text-gray-600">
                    <svg className="w-5 h-5 text-[#004E99] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      <section className="py-16 bg-gray-50 border-y border-gray-200">
        <div className="max-w-7xl mx-auto px-6">
          <h2 className="text-3xl font-bold text-[#051A2D] mb-12 text-center">Our Service Divisions</h2>
            ...
            {services.map((service, i) => (
              <div key={i} className={`grid lg:grid-cols-2 gap-8 items-center ${i % 2 === 1 ? "lg:flex-row-reverse" : ""}`}>
                ...
                <div className={i % 2 === 1 ? "lg:order-1" : ""}>
                  <h3 className="text-2xl font-bold text-[#051A2D] mb-4">{service.title}</h3>
                  <p className="text-gray-600 mb-6">{service.description}</p>
                  <div className="grid grid-cols-2 gap-3">
                    {service.features.map((feature, j) => (
                      <div key={j} className="flex items-center gap-2 text-sm text-gray-600">
                        <svg className="w-4 h-4 text-[#004E99]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                          <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                        </svg>
                        {feature}
                      </div>
      ...
      <section className="py-16 bg-white">
        <div className="max-w-5xl mx-auto px-6">
          <h2 className="text-3xl font-bold text-[#051A2D] mb-12 text-center">Our Claims Process</h2>
          ...
          <div className="grid md:grid-cols-4 gap-8">
            {[
              ...
              <div key={i}>
                <div className="w-10 h-10 bg-[#004E99] text-white rounded flex items-center justify-center mb-4 font-bold">
                  {item.step}
                </div>
      ...
      <section className="py-16 bg-gray-50 border-t border-gray-200">
        <div className="max-w-3xl mx-auto px-6">
          <h2 className="text-3xl font-bold text-[#051A2D] mb-12 text-center">Frequently Asked Questions</h2>
